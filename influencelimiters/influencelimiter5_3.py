...
class InfluenceLimiter5_3():
    ...
    def _compute_T_posterior(self, selected_arm, reward):
        ...
        alpha_tilde, beta_tilde = 1, 1

        alpha_tilde = (reward == 1) * self.reward_reports + alpha_tilde
        beta_tilde = (reward == 0) * self.reward_reports + beta_tilde

        #iterate through each agent and process their report
        for agent_index, agent in enumerate(self.agency.agents):

            alpha_j += self.agency.agent_reports[agent_index][selected_arm]*agent.num_reports
            beta_j += (1-self.agency.agent_reports[agent_index][selected_arm])*agent.num_reports
