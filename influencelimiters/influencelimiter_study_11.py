...
class InfluenceLimiter_study_11():
    ...
    def _update_reputations(self, arm, reward):
        ...
        for index, agent in enumerate(self.agency.agents):
            w = self.agent_reputations[agent]
            self.agent_reputations[agent] = w*np.exp(eta * (self.scoring_rule(reward, 0.5) - self.scoring_rule(reward, self.agency.agent_reports[agent][arm])))
