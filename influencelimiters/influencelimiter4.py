...
class InfluenceLimiter4():
    ...
    def __initialize_reputations(self):
        ...
        self.agent_reputations.append(self.initial_reputation)
        if self.track_reputation:
            self.agent_reputations_track = [[self.initial_reputation] for agent in self.agency.agents]
            self.agent_reputations_track.append([self.initial_reputation])
    ...
    def _compute_IL_posterior(self):
        ...
        for (arm_index, arm) in enumerate(self.bandit.arms):
            ...
            gamma = min(1, self.agent_reputations[-1])
            alpha_tilde = (1-gamma) * alpha_tilde + gamma*arm.reward_dist.mean()*self.reward_reports
            beta_tilde = (1-gamma) * beta_tilde + gamma*(1-arm.reward_dist.mean())*self.reward_reports
            self.posterior_history[arm_index].append(BetaDistribution(alpha_tilde, beta_tilde))
    ...
    def _update_reputations(self, arm, reward):
        ...
        gamma = min(1, self.agent_reputations[-1])
        q_tile_j_1 = self.posterior_history[arm][-2].mean()
        alpha_delta = self.bandit.arms[arm].reward_dist.mean() * self.reward_reports
        beta_delta = (1-self.bandit.arms[arm].reward_dist.mean()) * self.reward_reports
        # for i in range(index + 1):
        #     alpha_delta += self.agency.agent_reports[i][arm] * agent.num_reports * min(1, self.agency.agents[i].reputation)
        #     beta_delta += (1-self.agency.agent_reports[i][arm]) * agent.num_reports * min(1, self.agency.agents[i].reputation)
        ...
        # agent.reputation += gamma * (self.scoring_rule(reward, q_tile_j_1) - self.scoring_rule(reward, q_j))
        self.agent_reputations[-1] += gamma * (self.scoring_rule(reward, q_tile_j_1) - self.scoring_rule(reward, q_j))
        if self.track_reputation == True:
            self.agent_reputations_track[-1].append(self.agent_reputations[-1])
