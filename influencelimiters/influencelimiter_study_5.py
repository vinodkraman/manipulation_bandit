...
class InfluenceLimiter_study_5():
    ...
    def _update_reputations(self, arm, reward):
        ...
        for index, agent in enumerate(self.agency.agents):
            ...
            q_tile_j_1 = self.posterior_history[arm][index].get_quantile(0.95)
            q_j = self.prediction_history[arm][index].get_quantile(0.95)
    ...
    # def scoring_rule(self, r, dist):
    #     return -np.log(dist.get_pdf(r))
    #     # if r == 0:
    #     #     return (dist.alpha/(dist.alpha + dist.beta)) * (1 - 2*betainc(dist.alpha+1, dist.beta, r) - (2 * beta(2*dist.alpha, 2*dist.beta)/(dist.alpha*beta(dist.alpha, dist.beta)**2)))
    #     # else:
    #     #     return 1 + (dist.alpha/(dist.alpha + dist.beta)) * (1 - 2 - (2* beta(2*dist.alpha, 2*dist.beta)/(dist.alpha*beta(dist.alpha, dist.beta)**2)))
