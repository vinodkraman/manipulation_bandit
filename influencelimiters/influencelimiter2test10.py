...
class InfluenceLimiter2test10():
    ...
    def _compute_T_posterior(self, selected_arm, reward):
        for (arm_index, arm) in enumerate(self.bandit.arms):
            alpha_tilde, beta_tilde = copy.deepcopy(self.bandit.arms[arm_index].reward_dist.get_params())
            pre_alpha, pre_beta = copy.deepcopy(self.bandit.arms[arm_index].reward_dist.get_params())
            alpha_test, beta_test = 0, 0
            ...
            if arm_index == selected_arm:
                ...
                beta_tilde += (reward == 0) * self.reward_reports
            # print(alpha_test + alpha_tilde, beta_test + beta_tilde)
            self.bandit.arms[arm_index].reward_dist.set_params(alpha_test + alpha_tilde, beta_test + beta_tilde)
