...
class InfluenceLimiter2test6():
    ...
    def _compute_IL_posterior(self):
        ...
        for (arm_index, arm) in enumerate(self.bandit.arms):
            ...
            for agent_index, agent in enumerate(self.agency.agents):
                ...
                if (last_saved_alpha + last_saved_beta) == 0 and gamma >= 1:
                    last_saved_alpha = self.agency.agent_reports[agent_index][arm_index]*agent.num_reports
                    last_saved_beta = (1-self.agency.agent_reports[agent_index][arm_index])*agent.num_reports
                elif gamma >= 1:
                    alpha_test += last_saved_alpha
                    beta_test += last_saved_beta
    ...
    def _compute_T_posterior(self, selected_arm, reward):
        ...
        #     if agent_index > 0 and self.agent_reputations[agent_index-1] >= 1:
        #         # print(agent_index)
        #         alpha_test += self.agency.agent_reports[agent_index-1][selected_arm]*agent.num_reports
        #         beta_test += (1-self.agency.agent_reports[agent_index-1][selected_arm])*agent.num_reports

        #     alpha_j = self.agency.agent_reports[agent_index][selected_arm]*agent.num_reports + pre_alpha
        #     beta_j = (1-self.agency.agent_reports[agent_index][selected_arm])*agent.num_reports + pre_beta 
        #     self.prediction_history[selected_arm].append(BetaDistribution(alpha_j, beta_j))

        #     alpha_tilde = (1-gamma) * alpha_tilde + gamma*(alpha_j)
        #     beta_tilde = (1-gamma) * beta_tilde + gamma*(beta_j)
        #     self.posterior_history[selected_arm].append(BetaDistribution(alpha_tilde, beta_tilde))


        # self.bandit.arms[selected_arm].reward_dist.set_params(alpha_test + alpha_tilde, beta_test + beta_tilde)
